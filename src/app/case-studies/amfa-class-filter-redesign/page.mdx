...
export default function AMFAClassFilterRedesignCaseStudy() {
  ...
  const summary = {
    ...
    constraints: [
      'Three separate systems with no shared state',
      'Mixed audience (parents registering kids vs. adults)',
      'Stakeholder alignment needed across marketing, ops, and dev',
    ],
    decisions: [
      'Designed progressive disclosure filter (Youth/Adult binary split first, then age-appropriate options)',
      'Built overlay return pattern to preserve cross-system context',
      'Created GA4 instrumentation plan for measurement',
    ],
    impact: [
      'Support call volume down ~30\u201340% for checkout inquiries (metrics still being collected)',
      'Task completion trending upward across the filter-to-checkout funnel',
    ],
  }
