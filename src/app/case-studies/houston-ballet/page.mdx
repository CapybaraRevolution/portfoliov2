...
export default function HoustonBalletCaseStudy() {
  ...
  const summary = {
    ...
    constraints: 'One-day build window to validate the UX approach; designing for APIs that don\u0027t exist yet; multi-tenant scalability baked in from day one.',
    decisions: 'Built a 5-layer design token system (217 CSS custom properties); defined 40 TypeScript interfaces as API contracts before any backend existed; wrote 1,100+ lines of developer documentation across 6 handoff files.',
...
└─────────────┘     └──────────────┘     └──────────────┘`}
            ...
            <div className="px-4 py-3 bg-zinc-100/50 dark:bg-zinc-800/50 border-t border-zinc-200 dark:border-zinc-700">
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                My work is the left box. I defined the contracts so the middleware team can connect the pipes without guessing what the UI expects.
              </p>
            </div>
          </div>
        </FadeIn>
