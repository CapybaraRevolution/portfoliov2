...
export const caseStudies: CaseStudyMetadata[] = [
  ...
  },
  {
    ...
    status: "Completed",
    tools: ["Figma", "FigJam", "Miro", "Notion", "Google Analytics (GA4)"],
    ...
    goalDetail: "Three platforms, three teams, one registration flow. I unified the class discovery experience so people could find the right class and check out without picking up the phone."
  },
  {
...
export function getNextCaseStudy(currentSlug: string): CaseStudyMetadata | undefined {
  // Only navigate between published case studies (skip coming soon / under construction)
  const publishedStudies = [...caseStudies]
    .filter(s => !s.comingSoon && !s.underConstruction)
    .sort((a, b) => a.order - b.order)
  const currentIndex = publishedStudies.findIndex(study => study.slug === currentSlug)
  
  if (currentIndex === -1) return undefined
  
  // Wrap to first if on last
  const nextIndex = currentIndex + 1 >= publishedStudies.length ? 0 : currentIndex + 1
  return publishedStudies[nextIndex]
}
...
export function getPreviousCaseStudy(currentSlug: string): CaseStudyMetadata | undefined {
  ...
  const prevIndex = currentIndex - 1 < 0 ? publishedStudies.length - 1 : currentIndex - 1
  return publishedStudies[prevIndex]
}
